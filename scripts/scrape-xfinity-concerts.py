...
# -*- coding: utf-8 -*-
"""
Scraper for Xfinity Mobile Arena – Concerts events
URL: https://www.xfinitymobilearena.com/events/category/concerts
Tag: Music
"""
...
# CONFIG
# ─────────────────────────────────────────────
BASE_URL = "https://www.xfinitymobilearena.com/events/category/concerts"
SOURCE_NAME = "xfinitymobilearena-concerts"
...
def parse_date(text: str) -> str | None:
    """Clean and parse date formats like 'Dec. 14, 2025' or 'Feb. 10, 2026'."""
    if not text:
        return None
    try:
...
def parse_events():
    ...
    for div in soup.select(".eventItem.entry"):
        ...
        link_el  = div.select_one(".tickets") or (title_el and title_el["href"])

        if not title_el or not month_el or not day_el or not year_el:
            continue

        title = title_el.get_text(strip=True)
        link  = link_el if isinstance(link_el, str) else link_el["href"]
...
# ─────────────────────────────────────────────
def run():
    print("🎵 Scraping Xfinity Mobile Arena (Concerts)...")
