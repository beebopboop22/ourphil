...
export default function ThisWeekendInPhiladelphia() {
  ...
  const h1Suffix = `${formatWeekdayAbbrev(weekendStart, PHILLY_TIME_ZONE)}–${formatWeekdayAbbrev(weekendEnd, PHILLY_TIME_ZONE)}, ${weekendEnd.getFullYear()}`;
      ...
      <main className="flex-1 pt-28 pb-16">
        <div className="container mx-auto px-4 max-w-6xl">
          <h1 className="text-4xl sm:text-5xl font-[Barrio] text-[#28313e] text-center">
            Things to Do in Philadelphia This Weekend ({h1Suffix})
          </h1>
          <p className="mt-6 text-lg text-gray-700 text-center max-w-3xl mx-auto">
            Use this guide from the most comprehensive events calendar in Philadelphia to plan your {introRange} adventures. We curated festivals, markets, concerts, and family-friendly happenings so you can squeeze every drop out of the weekend.
          </p>

          <div className="mt-10 flex justify-end items-center gap-2">
            {hasFilters && (
              <button onClick={() => setSelectedTags([])} className="text-sm text-gray-500 hover:underline">
                Clear filters
              </button>
            )}
            <button
          ...
          <section className="mt-12">
            <div className="flex items-baseline justify-between flex-wrap gap-2">
              <h2 className="text-2xl font-semibold text-[#28313e]">
                {sortedEvents.length} picks for this weekend
              </h2>
