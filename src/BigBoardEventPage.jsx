...
export default function BigBoardEventPage() {
  ...
  const headerWhenLabel = useMemo(
    () => formatHeaderWhenLabel(event.start_date, event.start_time, event.end_time),
    [event.start_date, event.start_time, event.end_time]
  );

  const headerAreaName = useMemo(() => {
    ...
    return null;
  }, [event.area_id, areaLookup]);

  const headerAddressLine = useMemo(() => {
