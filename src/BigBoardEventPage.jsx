...
export default function BigBoardEventPage() {
  ...
  const metaItems = [
    formattedDateLabel && { key: 'date', content: formattedDateLabel },
    timeRangeText && { key: 'time', content: timeRangeText },
    areaName && {
      key: 'area',
      content: areaName,
      className: 'font-semibold text-indigo-600',
    },
    shortAddress && { key: 'address', content: shortAddress },
  ].filter(Boolean);
  ...
  return (
    <div className="flex flex-col min-h-screen bg-white">
      ...
      <main className="flex-grow bg-white">
        <div className="pb-16 pt-[11rem] sm:pt-[12rem] md:pt-32">
          <div
            className="sticky top-[10.5rem] z-30 border-b border-gray-200 bg-white/95 backdrop-blur supports-[backdrop-filter]:bg-white/80 sm:top-[9.5rem] md:top-[6.25rem]"
          >
                  ...
                  {!!metaItems.length && (
                    <div className="mt-1 flex flex-wrap items-center gap-x-4 gap-y-1 text-sm text-gray-600">
                      {metaItems.map((item, idx) => (
                        <span key={`${item.key}-${idx}`} className="flex items-center gap-2">
                          {idx > 0 && (
