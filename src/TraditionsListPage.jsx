...
export default function TraditionsListPage() {
  ...
  return (
    <div className="relative min-h-screen bg-[#bf3e35] text-white">
      <div className="pointer-events-none fixed inset-0 -z-10 overflow-hidden" aria-hidden="true">
        <div className="absolute inset-0">
          {pillConfigs.map((pill, index) => (
            <span
              key={`${pill.label}-${index}`}
              className="absolute rounded-full px-4 py-2 text-lg font-semibold text-white opacity-90 whitespace-nowrap select-none"
              style={{
                left: `${pill.left}%`,
                top: '-12vh',
