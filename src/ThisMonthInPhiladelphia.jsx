...
export default function ThisMonthInPhiladelphia() {
  const { monthSlug, year } = useParams();
  ...
  useEffect(() => {
    if (!monthIndex || Number.isNaN(yearNum)) {
      navigate('/philadelphia-events', { replace: true });
    }
  }, [monthIndex, yearNum, navigate]);
  ...
  useEffect(() => {
    ...
    const title = `Events in Philadelphia – ${monthLabel} (Traditions, Festivals, Markets)`;
    const description = `${monthLabel} traditions, festivals, family-friendly fun, concerts, and markets from the most comprehensive events calendar in Philadelphia.`;
  ...
  const countText = `${events.length} traditions in ${monthLabel}!`;
      ...
      <main className="flex-1 pt-28 pb-16">
        <div className="container mx-auto px-4 max-w-5xl">
          <h1 className="text-4xl sm:text-5xl font-[Barrio] text-[#28313e] text-center">
            Events in Philadelphia: {monthLabel}
          </h1>
          <p className="mt-6 text-lg text-gray-700 text-center max-w-3xl mx-auto">
            Welcome to the most comprehensive events calendar in Philadelphia. Explore {monthLabel} traditions covering markets, festivals, concerts, and family-friendly outings so your plans stay rich all month long.
          </p>
          ...
          {!user && (
            <div className="mt-16 text-center">
              <Link
                to="/signup"
                className="inline-flex items-center px-6 py-3 bg-indigo-600 text-white rounded-full font-semibold hover:bg-indigo-700 transition"
              >
