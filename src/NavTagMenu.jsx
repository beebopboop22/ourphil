      ...
      <div className="flex items-center px-4 py-1">
        {tags.map((tag, i) => (
          <Link
            key={tag.slug}
            to={`/tags/${tag.slug}`}
            className={`${pillStyles[i % pillStyles.length]} text-xs font-semibold px-2 py-0.5 mr-2 rounded-full flex-shrink-0 hover:opacity-80 transition`}
          >
        ...
        {seasonalTags.map(tag => (
          <Link
            key={tag.slug}
            to={`/tags/${tag.slug}`}
            className="bg-gradient-to-r from-yellow-200 via-yellow-300 to-yellow-400 text-yellow-900 text-xs font-semibold px-2 py-0.5 mr-2 rounded-full border border-yellow-600 flex-shrink-0 hover:opacity-80 transition"
          >
            #{tag.name.toLowerCase()}
          </Link>
        ))}
