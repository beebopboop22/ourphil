...
export default function GroupMatchWizard({ onClose, onAddGroup }) {
  ...
  useEffect(() => {
    supabase
      ...
      .then(({ data }) => {
        ...
        setAllTags(['All', ...Array.from(types).sort()]);
      });
  }, []);
  ...
  const findMatches = async () => {
    // Require at least one tag or an area before searching
    ...
    const results = (groupData || [])
      .map(g => {
        ...
        const overlap = allSelected
          ? groupTags.length
          : selectedTags.length
          ? groupTags.filter(t => selectedTags.includes(t)).length
          : 0;
        // If specific tags were chosen, require overlap; if only an area was
        // selected, filter strictly by that area
        if (!allSelected && selectedTags.length && overlap === 0) return null;
        if (!allSelected && !selectedTags.length && selectedArea && g.Area !== selectedArea)
          return null;
        ...
        {matches === null ? (
          <div>
            ...
            <div className="flex justify-between">
              ...
              <button
                onClick={findMatches}
                disabled={!allSelected && !selectedTags.length && !selectedArea}
