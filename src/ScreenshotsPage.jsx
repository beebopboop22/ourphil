...
const SOURCE_STYLES = {
  ...
  big_board_events: 'bg-purple-100 text-purple-900',
  all_events: 'bg-blue-100 text-blue-900',
  group_events: 'bg-emerald-100 text-emerald-900',
  recurring_events: 'bg-orange-100 text-orange-900',
};
...
const formatDatePill = (date, timeStr) => {
  if (!date) return '';
    ...
    const loadEvents = async tagSlug => {
      ...
      try {
        ...
        const [eventsRes, bigBoardRes, allRes, groupRes, recurringRes] = await Promise.all([
          eventsQuery ?? Promise.resolve({ data: [] }),
          bigBoardQuery ?? Promise.resolve({ data: [] }),
          allEventsQuery ?? Promise.resolve({ data: [] }),
          groupEventsQuery ?? Promise.resolve({ data: [] }),
          recurringQuery ?? Promise.resolve({ data: [] }),
        ]);
        ...
        if (groupRes?.data?.length) {
          ...
          if (groupIds.length) {
            const { data: groupsData = [] } = await supabase
              ...
              .select('id,slug')
              .in('id', groupIds);
            groupSlugMap = groupsData.reduce((acc, row) => {
              acc[row.id] = row.slug;
              return acc;
            }, {});
          }
        ...
        (groupRes?.data || []).forEach(row => {
          ...
          if (!start) return;
          if (end && end < today) return;
          const groupSlug = row.group_id ? groupSlugMap[row.group_id] : null;
          const detailPath =
            getDetailPathForItem({
              ...row,
        ...
        const upcoming = merged
          ...
          .map(item => {
            ...
            return {
              ...
              href,
              isExternal,
              dateLabel: formatDatePill(item.start, item.start_time),
                ...
                <div className="flex flex-col gap-6 md:flex-row md:items-center">
                  ...
                  <div className="flex-1">
                    <div className="mb-4 flex flex-wrap items-center gap-3 text-lg md:text-xl">
                      {event.dateLabel && (
                        <span className="rounded-full bg-slate-900 px-4 py-2 font-semibold text-white">
                          {event.dateLabel}
                        </span>
                      )}
                      {event.areaName && (
                        <span className="rounded-full bg-slate-100 px-4 py-2 font-semibold text-slate-800">
                          {event.areaName}
                        </span>
                      )}
                      {typeLabel && (
                    ...
                    {event.venue_name && (
                      <p className="text-lg text-slate-600">at {event.venue_name}</p>
                    )}
                  </div>
                </div>
              </Wrapper>
