...
export default function EventLocationMap({ latitude, longitude, eventName, locationLabel }) {
  ...
  const heading = eventName ? `Where ${eventName} is happening` : "Where it's happening";
  const subtitle =
    locationLabel && locationLabel !== 'Philadelphia'
      ? locationLabel
      : 'Zoom in to explore the block.';

  return (
    <section className="max-w-4xl mx-auto mt-16 px-4">
      <h2 className="text-3xl sm:text-4xl font-[Barrio] text-gray-800 mb-4">{heading}</h2>
      <div className="relative w-full h-[360px] sm:h-[420px] rounded-3xl overflow-hidden shadow-2xl border border-gray-200">
        ...
        <div className="absolute bottom-4 left-4 bg-black/70 text-white px-4 py-3 rounded-xl max-w-[80%] backdrop-blur">
          <p className="text-sm font-semibold leading-snug">{eventName || 'Event location'}</p>
          {subtitle && (
            <p className="text-xs text-white/80 mt-1 leading-snug">{subtitle}</p>
          )}
        </div>
      </div>
    </section>
